...
const Friends: React.FC = () => {
  ...
  const loadFriends = async () => {
    ...
    try {
      ...
      console.log('[DEBUG] friendAPI.list() result:', friendsData);
      
      // APIレスポンスの構造を確認して適切に処理
      if (friendsData && typeof friendsData === 'object' && 'friends' in friendsData) {
        // { friends: [...] } の形式の場合
      ...
      if (requestsData && typeof requestsData === 'object' && 'requests' in requestsData) {
        setFriendRequests(Array.isArray(requestsData.requests) ? requestsData.requests : []);
